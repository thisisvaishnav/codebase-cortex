...
- [ ] **Initialize TrueForge Core Harness & MCP Connectors** - Configure `agent.json` with model provider, tools, and subagents
  ...
  - [ ] Rewrite `agent.json` to the real `AgentSpec` schema (current file matches no field of it) — BLOCKED on direction
  - [ ] Configure GitHub MCP connector (`readDiff`, `postComment`, `upsertComment`, `readIssue`, `readCODEOWNERS`) — only the **remote** `github` MCP is usable (`api.githubcopilot.com/mcp/`); stdio unsupported
  - [ ] ~~Configure VectorStore MCP connector~~ — **not possible as MCP** (no remote Chroma server; stdio unsupported). Needs redesign
  - [ ] ~~Configure Filesystem MCP connector~~ — **not possible as MCP** (no filesystem server in catalog). Must use the sandbox
...
- [ ] **Build Vector Knowledge Store & ADR Ingestion Engine** - Set up ChromaDB schema and embedding pipeline
  - Initialize ChromaDB local/hosted client with vector collections (`adr_collection`, `diff_embeddings`)
  - Implement ADR schema validator (`id`, `title`, `author`, `status`, `scope_files`, `invariants`, `reasoning`, `date`)
  - Build cold-start indexer to load existing ADRs (`docs/adr/ADR-*.md`) into vector collection
  - Implement cosine similarity query interface with threshold calibration (>= 0.70 retrieval)

- [ ] **Implement `cortex-ingest` Agent & PR Merge Pipeline** - Automate institutional memory capture on merge
  - Build PR decision template parser (`.github/pull_request_template.md` structured fields)
  - Implement fallback LLM extractor for PRs lacking explicit decision template
  - Implement Git ADR writer saving markdown records to `docs/adr/ADR-XXX-<slug>.md`
  - Implement ADR status transition logic (mark previous decisions as `SUPERSEDED` when overridden)
  - Wire automated post-merge confirmation comment to GitHub PR thread
...
- [ ] **Configure GitHub Actions CI Automation Workflows** - Zero-dependency headless execution in CI
  - [x] Build `.github/workflows/cortex-detect.yml` triggering on `pull_request` (opened, synchronize) and `issues` (opened) — file exists, **but its run step is broken** (calls a nonexistent `trueforge run` CLI)
  - [x] Build `.github/workflows/cortex-ingest.yml` triggering on `pull_request` (closed, merged=true) — file exists, **same broken run step**
  - [ ] Replace both run steps with a working invocation path (REST API, or our own CLI wrapper) — BLOCKED on direction
  - [ ] Configure GitHub secrets (`TF_API_KEY`, `GITHUB_TOKEN`, `CHROMA_API_KEY`) and runner caching
